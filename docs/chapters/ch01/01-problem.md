...
> **[交互可视化：问题澄清工作台]**
> 
> 想象一个界面，左边是你收到的任务描述（可以是自然语言），右边是需要澄清的问题列表：
> - 输入格式是什么？（文本框，你可以输入示例）
> - 输出格式是什么？（文本框，你可以输入示例）
> - 数据规模多大？（滑块，从 10 到 10亿）
> - 时间限制多久？（滑块，从毫秒到分钟）
> - 有没有特殊情况需要处理？（多选：空输入、极端值、重复元素...）
>
> 填完这些，系统会生成一个清晰的问题描述，你可以直接发给 agent。
...
> 
> 一个从左到右的流程图：
> - 左边：模糊的任务描述（用气泡表示，里面的文字是"帮我写一个搜索"）
> - 中间：三个问题框（输入输出、结构、约束），每个框有箭头指向它
> - 右边：清晰的规格描述（用文档图标表示）
>
> 图的风格是简洁的手绘风，颜色用蓝色和灰色。箭头要明显，表示这是一个"澄清"的过程。
