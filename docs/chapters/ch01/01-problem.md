...
搜索问题的特征是"在一堆东西里找目标"。如果数据是无序的，你可能需要一个个检查。如果数据已经有序，你可以每次检查中间位置、排除一半范围。如果需要支持模糊匹配，你可能需要提前建立"关键词到商品"的查找表。

排序问题的特征是"让一堆东西有序"。如果数据量小，简单方法就够了。如果数据量大，你需要增长更慢的排序方法。如果对稳定性有要求，也就是相等元素的原有顺序不能被打乱，有些排序方法就不能用了。
...
后来他找到了原因。原来 Copilot 给他的代码使用了一种递归排序：每次选一个基准元素，把数据分成"比它小"和"比它大"两边，再分别排序。但这段实现总是选择固定位置的元素作为基准。昨晚的商品数据恰好接近有序，导致每次划分都极不平衡。于是递归深度接近 n，不仅时间复杂度退化到 O(n²)，还触发了 Java 的栈溢出。
