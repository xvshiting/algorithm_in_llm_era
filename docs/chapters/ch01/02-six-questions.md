...
| 识别信号 | 可能的问题类型 | 追问 |
|---------|---------------|------|
| 有节点和边 | 图遍历、连通性、最短路、网络流 | 边是否有方向和权重？ |
| 要找"最短、最快、最低成本" | 最短路、动态规划、贪心 | 局部最优是否可靠？ |
| 要安排、匹配、分配 | 匹配、流、贪心、优化问题 | 是一对一、多对一，还是容量受限？ |
| 要从序列中找规律 | 排序、查找、动态规划 | 是否有重叠子问题？ |
| 要在大量数据中判断是否出现过 | 哈希、布隆过滤器、索引 | 是否允许概率性错误？ |
| 要实时处理连续数据 | 流式算法、滑动窗口、采样 | 能否保存全部历史？ |
...
|------|---------|------------------------|
| 分治 | 子问题是否独立？合并是否便宜？ | 子问题重叠导致重复计算 |
| 贪心 | 局部最优为什么能推出全局最优？ | 得到看似合理但错误的答案 |
| 动态规划 | 状态是什么？转移是否覆盖所有情况？ | 状态爆炸或遗漏边界 |
| 回溯 | 搜索空间是什么？剪枝是否安全？ | 枚举失控或剪掉正确答案 |
| 网络流 | 容量、源点、汇点如何对应现实约束？ | 建模错位，算法正确但问题错误 |
| 随机化 | 允许什么概率性错误？如何控制失败率？ | 把不确定结果误当确定结果 |
| 流式算法 | 能保存多少状态？是否允许近似？ | 用单机批处理思路处理实时流 |

例如，看到"每一步选当前最优"并不能说明可以用贪心。你还要说明：为什么这个局部选择不会破坏未来的全局最优？如果说不清，就应该怀疑贪心。
