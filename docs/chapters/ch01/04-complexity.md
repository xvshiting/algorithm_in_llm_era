...
下面这些量级会反复出现。现在不需要背算法名，先掌握它们的代码形状和规模后果。

| 量级 | 典型形状 | 当 `n = 100,000` 时的大致步骤数 |
|------|----------|-------------------------------|
| `O(1)` | 只做固定次数操作 | 约 1 |
| `O(log n)` | 每一步把待处理范围缩小为原来的一部分 | 约 17 |
| `O(n)` | 看一遍所有输入 | 100,000 |
| `O(n log n)` | 分层处理，每层总共看一遍输入 | 约 1,700,000 |
| `O(n²)` | 两两配对或双层遍历 | 10,000,000,000 |
| `O(2^n)` | 每个元素都有取或不取等分支 | 极大，通常不可接受 |
