...
| 项目 | 要回答的问题 | 产出 |
|------|--------------|------|
| 任务 | 谁要用这个结果？用来做什么决策？ | 真实目标 |
| 输入 | 已知数据有哪些？格式、单位、范围是什么？ | 输入定义 |
| 输出 | 要返回一个判断、数值、列表、安排，还是一组选择？ | 输出定义 |
| 硬约束 | 哪些条件必须满足，不满足就不能接受？ | 合法性规则 |
| 目标 | 在合法答案里，如何比较哪个更好？ | 评分规则 |
| 规模 | 输入大概多大？峰值多大？ | 复杂度预算 |
| 边界 | 空输入、相等、重复、缺失、非法数据怎么处理？ | 边界约定 |
| 例子 | 一个正常例子和一个容易出错的例子是什么？ | 校验样例 |

这张卡不是文档负担。它是在防止你过早问"用什么算法"。

没有建模卡时，算法选择很容易变成猜谜：工具猜你的意思，你猜工具有没有猜对。
...
工具可以帮你列澄清问题，但不能替你知道现实中谁会拒绝这个结果。这个判断来自业务、用户、物理世界和责任边界。
