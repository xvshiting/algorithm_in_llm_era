...
2022年，某日志处理系统在升级后出现了奇怪的性能问题。日志写入量增加了 30%，但处理延迟增加了 10 倍。
...
def add_log(entry):
    logs.append(entry)  # 入队 - 末尾追加

def get_next():
    return logs.pop(0)  # 出队 - 取第一条
```
...
```
当队列有 10000 条日志时：
- pop(0) 删除第一条后，后面 9999 条都要向前移动一个位置
- 每移动一条日志约 1 微秒
- pop(0) 单次耗时约 10 毫秒

当队列有 100000 条日志时：
- pop(0) 单次耗时约 100 毫秒
- 处理 100000 条日志的总时间约 5 秒变成 50 秒
```

**根本原因**：用数组做队列，头部删除每次都要移动几乎全部元素。
...
def get_next():
    return logs.popleft()  # Θ(1)，无需移动元素
```

修复后，100000 条日志的处理时间从 50 秒降回 5 秒。
...
| 队列长度 | pop(0) 单次耗时 | 处理 n 条的总时间 |
|----------|-----------------|-------------------|
| 1,000 | ~1 ms | ~0.5 秒 |
| 10,000 | ~10 ms | ~5 秒 |
| 100,000 | ~100 ms | ~50 秒 |
| 1,000,000 | ~1 秒 | ~5000 秒 (83 分钟) |
