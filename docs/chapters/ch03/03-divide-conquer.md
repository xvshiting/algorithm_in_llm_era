...
3. **判断什么时候快排会被攻击**，理解确定性算法的脆弱性
4. **审查 agent 给出的排序实现**，发现隐藏的 Θ(n²) 风险
...
你可能会问：为什么归并排序的分解那么简单，快排的分解那么复杂？
...
- 如果 A[j] ≤ pivot：i++, swap。A[j] 进入 ≤ pivot 区域。
...
关键洞见：**两个元素被比较的唯一条件是，它们中第一个被选为 pivot**。

```
E[比较次数] = Σ_{i<j} P(i和j被比较)
            = Σ_{i<j} 2/(j-i+1)
            ≈ 2n ln n
            = Θ(n log n)
```

**结论**：期望 Θ(n log n)，最坏 Θ(n²)。
...
**练习 4**：写出 PARTITION 的循环不变式，并完整证明三步。

**练习 5**：设计一个 MERGE 的变体，合并三个有序数组而不是两个。分析复杂度。
