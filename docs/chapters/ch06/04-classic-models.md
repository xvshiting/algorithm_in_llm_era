...
\`\`\`python
def dp_framework(X, Y, row_init, column_init, match, goal_cell):
    m, n = len(X), len(Y)
    dp = [[0] * (n + 1) for _ in range(m + 1)]
...
def column_init(dp, m):
    """初始化第一列"""
    pass

def match(c1, c2):
    """匹配代价（0 表示匹配，>0 表示替换代价）"""
    pass

def goal_cell(dp, m, n):
    """终点位置，返回答案"""
    pass
\`\`\`
...
\`\`\`python
def cut_rod_memo(price, n, memo=None):
    if memo is None:
        memo = {}
    if n in memo:
        return memo[n]
...
朴素算法：对于长度为 l 的区间，搜索范围是 l-1 个位置。
优化后：搜索范围被限制在 s[i][j-1] 到 s[i+1][j]，平均只有常数个位置。
...
\`\`\`python
def edit_distance_with_operations(X, Y):
    m, n = len(X), len(Y)
    dp = [[0] * (n + 1) for _ in range(m + 1)]
...
\`\`\`python
def weighted_match(c1, c2, insert_cost, delete_cost, replace_cost):
    if c1 == c2:
        return 0
    else:
        return replace_cost
...
4. **编辑距离实现**：实现编辑距离，并扩展支持不同操作代价。

5. **LCS 空间优化**：实现 LCS 的滚动数组版本。

6. **钢条切割路径重构**：实现钢条切割的最优方案输出。

### 错误诊断

7. **诊断错误**：给定一个"忘记初始化"的 DP 代码，预测错误。

8. **索引错误**：如果 DP 表索引与字符串索引不一致，会发生什么？

9. **Knuth 误用**：有人在不满足单调性的问题上用 Knuth 优化，分析后果。

### 方案比较

10. **编辑距离 vs LCS**：对比两个问题的桩函数差异。

11. **Knuth vs 普通矩阵链乘**：对比两种实现的时间复杂度。

12. **路径重构方法**：对比辅助表和回溯两种方法。

### 开放设计

13. **新问题框架设计**：为"最长公共子串"设计可插拔框架。

14. **Knuth 条件验证**：验证矩阵链乘是否满足 Knuth 优化条件。

15. **LLM 连接**：设计一个 Prompt 的可插拔框架，类比 DP 框架。

### 综合应用

16. **最优 BST 实现**：实现最优 BST 的 DP，并输出最优树结构。

17. **组合问题**：设计一个新问题，用可插拔框架解决。

18. **真实问题**：用 DP 解决实际问题：最优股票买卖（最多 k 次交易）。
