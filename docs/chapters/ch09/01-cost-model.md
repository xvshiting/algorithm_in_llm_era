...
**问题**：100台机器，每台1GB数据，求全局Top-100。

**方案A：中心聚合**
```
传输：100台 × 1GB = 100GB → 中心
计算：中心排序 → Top-100
瓶颈：网络传输、中心排序
时间：传输100GB ≈ 3小时（1Gbps）
```

**方案B：本地Top-K + 合并**
```
本地计算：每台求本地Top-100 → O(n)时间
传输：100台 × 100条 × 20字节 = 200KB
中心合并：合并10000条 → Top-100
时间：毫秒级
```

**对比**：
- 方案A：功能正确，但传输100GB → 系统不可用
- 方案B：功能正确，传输200KB → 可用
