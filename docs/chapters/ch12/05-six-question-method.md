...
**LLM时代关联**：LLM Agent诊断框架：六问法可直接编码为Agent的诊断流程。
...
**LLM时代关联**：LLM理解用户需求：将模糊问题转化为明确的算法问题。
...
**LLM时代关联**：LLM先写朴素解法：从暴力方法开始，再逐步优化。
...
**LLM时代关联**：LLM查阅知识库：检索相关算法知识，辅助问题求解。
...
**LLM时代关联**：LLM匹配算法范式：识别问题类型，选择合适的算法模板。
...
**LLM时代关联**：LLM迭代优化：根据执行结果调整策略，循环改进。
